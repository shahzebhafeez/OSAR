...
# --- CORRECTED with realistic PDR and Overhead logic, professional plotting, and interactive tooltips ---
...
class SimulationApp:
    ...
    def run_simulation_thread(self, p):
        ...
        while True:
            ...
            # If no forwarder is found, the packet is dropped.
            if best_nbr is None:
        ...
        # PDR Calculation
        pdr = packets_delivered / 1.0 # 1 packet was generated
        self.q.put({'type': 'log', 'message': f"Packet Delivery Ratio (PDR): {pdr:.0%}"})
        
        # Overhead Ratio Calculation (Refined Logic)
        # Control packets = 1 beacon per hop. Data packets = 1 data packet per hop.
        num_hops = max(len(route) - 1, 0)
        num_control_packets = num_hops
        num_data_packets = num_hops
        
        if (num_control_packets + num_data_packets) > 0:
            overhead_ratio = num_control_packets / (num_control_packets + num_data_packets)
        else: # Handles case where source can't even make one hop
            overhead_ratio = 0.0
