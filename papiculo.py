...
# --- FINAL VERSION with corrected overhead for failed attempts ---
...
CONTROL_PACKET_LP = 64 # Control packet size in bits (8 bytes), for realistic overhead
CH_BANDWIDTH = 6e3  # 6 kHz bandwidth per channel
DEFAULT_PT_DB = 150.0 # Default transmit power in dB re uPa
AREA = 500.0
TX_RANGE = 250.0
P_BUSY = 0.8 # Increased default for more challenging conditions
N_NODES = 30
M_SUBCARRIERS = 128
...
class SimulationApp:
    ...
    def hover(self, event):
        ...
        if event.inaxes == self.ax and self.sc:
        ...
        try:
            while True:
                ...
                if data['type'] == 'log':
                    ...
                    self.ax.scatter(data['dest'][0], data['dest'][1], data['dest'][2], c="red", s=180, marker="^", 
    ...
    def run_simulation_thread(self, p):
        ...
        packets_delivered, total_data_bits, total_control_bits, total_end_to_end_delay = 0, 0, 0, 0.0

        for i in range(packets_generated):
            self.q.put({'type': 'log', 'message': f"\n--- Routing Packet {i+1}/{packets_generated} ---"})
            ...
            for hop_count in range(p['N_NODES'] + 5):
                ...
                if best_nbr is None:
                    # The cost of the failed request is now correctly accounted for.
                    self.q.put({'type': 'log', 'message': f"  ⚠️ Node {current.node_id} is stuck. Dropping packet."})
                    break
                
                # 2. If successful, account for the "reply" control packet and the data packet.
                total_control_bits += CONTROL_PACKET_LP
                total_data_bits += LP
                packet_delay += TD
        ...
        self.q.put({'type': 'log', 'message': f"Packet Delivery Ratio (PDR): {pdr:.2%} ({packets_delivered}/{packets_generated})"})
        
        total_bits = total_data_bits + total_control_bits
        overhead_ratio = total_control_bits / total_bits if total_bits > 0 else 0
        self.q.put({'type': 'log', 'message': f"Overhead Ratio (by bytes): {overhead_ratio:.4f}"})
        
        avg_delay = total_end_to_end_delay / packets_delivered if packets_delivered > 0 else float('inf')
        self.q.put({'type': 'log', 'message': f"Average End-to-End Delay (for delivered packets): {avg_delay:.4f} s"})
