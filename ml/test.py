...
class SimulationApp:
    ...
    def setup_logging(self):
        ...
        try:
            ...
            fh.setFormatter(logging.Formatter('%(asctime)s [%(threadName)-15s] %(message)s', '%H:%M:%S'))
    ...
    def toggle_simulation(self):
        if self.is_simulation_running(): self.stop_simulation()
    ...
    def start_simulation(self):
        ...
        self.init_hover_functionality(); self.auv_artists.clear(); self.sc_nodes = None
        self.simulation_log_data.clear(); self.export_button.config(state=tk.DISABLED)
        self.auvs.clear(); self.sim_nodes.clear(); self.osar_results = None; self.auv_results = None
        
        try:
            p = {key: float(var.get()) for key, var in self.params.items()}
            p["N_NODES"] = int(p["N_NODES"]); p["NUM_AUVS"] = int(p["NUM_AUVS"])
            p["NUM_PACKETS"] = int(p["NUM_PACKETS"]); p["M_CHANNELS"] = int(p["M_CHANNELS"])
            p["SEED"] = int(p["SEED"]); p["PT_LINEAR"] = 10**(p["PT_DB"] / 10.0)
            ...
            self.log(f"Error: {e}"); traceback.print_exc()
    ...
    def run_concurrent_simulation(self, p):
        try:
            ...
            self.auv_thread = threading.Thread(target=self.run_auv_thread, args=(p,), daemon=True, name='AUV-Thread')
            self.mc_thread = threading.Thread(target=self.run_mc_logic, args=(p,), daemon=True, name='MC-Thread')
            self.packet_thread = threading.Thread(target=self.run_packet_routing_thread, args=(p,), daemon=True, name='Packet-Thread')

            self.auv_thread.start(); self.mc_thread.start(); self.packet_thread.start()
            self.packet_thread.join()
            self.simulation_running.clear()
            self.auv_thread.join(timeout=1.0); self.mc_thread.join(timeout=1.0)
        except Exception as e:
            self.log(f"Error: {e}"); traceback.print_exc()
        finally:
            self.simulation_running.clear()
    ...
    def run_sweep_thread(self):
        try:
            ...
            for auv_count in sweep_counts:
                ...
                # --- PART 1: WITH LC/OC (Uses user drift) ---
                temp_metrics_with = []
                for i in range(RUNS_PER_POINT):
                    ...
                    current_p['NODE_DRIFT'] = user_drift # Allow drift if user set it
                ...
                else: final_results_with.append((0,0,0,0))

                # --- PART 2: WITHOUT LC/OC (Forced Zero Drift) ---
                if self.stop_requested: break
                
                current_p = p.copy()
                current_p['NUM_AUVS'] = auv_count
                current_p['NODE_SEED'] = base_seed
                current_p['USE_AUVS'] = False 
                
                # --- FIX: FORCE DRIFT TO ZERO FOR BASELINE STABILITY ---
                current_p['NODE_DRIFT'] = 0.0 
                # -----------------------------------------------------
                ...
                self.simulation_running.set()
                self.logger.info(f"Completed {auv_count} AUVs. Avg PDR (With): {final_results_with[-1][0]:.2%}")
            ...
            self.logger.error(f"Sweep Error: {e}"); traceback.print_exc()
        finally:
            self.simulation_running.clear()
        ...
        try:
            while True:
                ...
                if data['type'] == 'log': self.log_text.insert(tk.END, data['message'] + "\n"); self.log_text.see(tk.END)
                    ...
                    self.plot_comparative(data['x'], data['y_with'], data['y_without'])
    ...
    def plot_initial_setup(self, nodes_pos, buoy_pos, area):
        ...
        if nodes_pos.any(): self.sc_nodes = self.ax.scatter(nodes_pos[:,0], nodes_pos[:,1], nodes_pos[:,2], c=nodes_pos[:,2], cmap='viridis_r')
        self.ax.scatter(buoy_pos[0], buoy_pos[1], buoy_pos[2], c='red', marker='^', s=200)
        self.ax.set_xlim(0, area); self.ax.set_ylim(0, area); self.ax.set_zlim(area, 0)
        self.fig.canvas.draw_idle()

    def update_node_plots(self, nodes_pos):
        if self.sc_nodes: self.sc_nodes._offsets3d = (nodes_pos[:,0], nodes_pos[:,1], nodes_pos[:,2])
    ...
    def update_auv_plots(self, indices):
        for i in indices:
            if i in self.auv_artists and i < len(self.auvs):
                auv = self.auvs[i]; path = np.array(auv.traveled_path); pos = auv.current_pos
                if len(path) > 1: self.auv_artists[i]['path'].set_data_3d(path[:,0], path[:,1], path[:,2])
                self.auv_artists[i]['marker'].set_data_3d([pos[0]], [pos[1]], [pos[2]])
    ...
    def hover(self, event):
        if event.inaxes == self.ax and self.sc_nodes:
            ...
            if cont:
                ...
                self.annot.xy = pos; self.annot.set_text(f"Pos: {pos}")
                self.annot.set_visible(True); self.fig.canvas.draw_idle()
            else:
                if self.annot.get_visible(): self.annot.set_visible(False); self.fig.canvas.draw_idle()

    def collect_auv_features(self):
        features = []; auv_map = {}
        if not self.auvs: return None, None
        for i, auv in enumerate(self.auvs):
            count = sum(1 for n in self.sim_nodes if auv_distance(auv.current_pos, n.pos) < auv.coverage_radius)
    ...
    def run_auv_thread(self, p):
        try:
            ...
            self.q.put({'type': 'setup_auv_plots', 'num_auvs': len(local_auvs)})
            ...
            while self.simulation_running.is_set() and sim_t < dur:
                ...
                for auv in local_auvs:
                    _, is_moving, _ = auv.update(AUV_UPDATE_INTERVAL_S, self.sim_nodes)
    ...
    def run_packet_routing_thread(self, p):
        ...
        while not self.sim_nodes and self.simulation_running.is_set(): time.sleep(0.1)
        if not self.sim_nodes: return
        
        sorted_nodes = sorted(self.sim_nodes, key=lambda n: n.depth, reverse=True)
        src_pool = sorted_nodes[:5]
        pkt_del = 0; d_bits = 0; c_bits = 0; tot_delay = 0.0
        
        is_sweep = self.sim_speed_multiplier > 1.0
        
        if is_sweep:
            dur = p.get('SWEEP_DURATION', SWEEP_DURATION_S); interval = SWEEP_PACKET_INTERVAL_S
            sim_t = 0.0; pkt_count = 0
            while self.simulation_running.is_set() and sim_t < dur:
                ...
                if res: pkt_del += 1; d_bits += LP; c_bits += res[1]; tot_delay += res[0]
                else: c_bits += CONTROL_PACKET_LP * int(p['N_NODES'])
            pkt_gen = pkt_count
        else:
    ...
    def route_single_packet(self, src, p, freqs, rng):
        curr = src; path = [src.pos]; delay = 0.0; ctrl_bits = 0
